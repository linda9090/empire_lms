...
시스템 설치 후 기본 관리자 계정이 생성됩니다:

| 항목 | 값 |
|------|-----|
| **이메일** | admin@empire-lms.com |
| **비밀번호** | Admin1234! |
| **역할** | ADMIN |

### 관리자 계정 생성

기본 관리자 계정은 데이터베이스 시드 스크립트로 생성됩니다:
...
1. **비밀번호 변경**: 보안을 위해 기본 비밀번호를 즉시 변경하세요
2. **이메일 인증**: 이메일 인증 설정을 확인하세요
3. **2FA 활성화**: 가능한 경우 2단계 인증을 활성화하세요

### 일반 사용자 계정

일반 사용자 계정은 웹 인터페이스를 통해 생성할 수 있습니다:
